...
public class TreeMapServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			File dir=new File(path+"/data","treey.json");
			if(!dir.exists()){
				dir.createNewFile(); 
			}
			BufferedWriter writer = new BufferedWriter(new OutputStreamWriter( 
					new FileOutputStream(path+"/data/treey.json"), "utf-8")); 
			writer.write(treejson);
			writer.close();
			request.getRequestDispatcher("/WEB-INF/jsp/treemap.jsp").forward(request, response);
		} catch (Exception e) {
